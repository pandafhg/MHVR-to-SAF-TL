...
if os.path.exists("%s/TL_CV1.onnx" % os.getcwd()):
    model_file = "%s/TL_CV1.onnx" % os.getcwd()
# If the model file does not exist in the current directory, 
# please input the full path of the model.onnx.
else:
    model_file = input("input the file path of *.onnx (/*.onnx):")
...
for test_file in get_file(mhvr_loc):
    ...
    if os.path.splitext(name_file)[1] == '.csv':
        result.to_csv("%s/SAF_%s" % (y_folder, name_file), float_format = '%.3f', sep = ',',index=None)
